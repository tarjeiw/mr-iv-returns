...
tempname b
 ...
 qui forv a = 17(1)62 {
  foreach y in linntW {
  xtset tkom
  xi: xtivreg2 `y'_a`a' yos i.cohort, fe
  scalar est`a' = _b[yos]
  }
 }
...
*Panel: Internal rate of return (bootstrap SEs with 250 reps)
use "lt_final_estsample_iv.dta", clear
 ...
 *bootstrap sampling over ages 25-44
 qui forv a = 25(1)44 {
  foreach y in linntW {
  xtset tkom
  xi: xtivreg2 `y'_a`a' (yos=piv) i.cohort, fe
  scalar est`a' = _b[yos]
  }
  }
...
qui foreach t in a1724 a4552 {
 foreach y in mlt_linntW {
 xtset tkom
 xi: xtivreg2 `y'_`t' (yos=piv) i.cohort, fe
...
**# 1. OLS
u "lt_final_estsample_ols.dta", clear 
keep if inrange(cohort,1959,1967) // balanced panel ages 17-55 
** Internal rate of return (bootstrap SEs with 250 reps)
cap program drop irr
program irr, eclass
version 12
tempname b
...
**# 4. MR
use "lt_final_estsample_iv.dta", clear
keep if inrange(cohort,1959,1970) // balanced panel ages 17-52 
* Internal rate of return (bootstrap SEs with 250 reps)
cap program drop irr_fix
program irr_fix, eclass
cap syntax [if] [, fixest1(real -23444) fixest2(real 72083)]
